...
#: Rebuilds the profile tables for a database created while `profile_kind` was
#: an enumerated CHECK, carrying every row across. Applied by `_init_db` only
#: when the live DDL still carries one.
    ...
    def _init_db(self) -> None:
        with self.connect() as con:
            with con:
                ...
                # A kind is open, and SQLite cannot alter a CHECK. The live DDL
                # says whether one is still there, so it decides whether the
                # rebuild is owed — a database built from the open SCHEMA skips
                # this entirely.
                ddl_row = con.execute(
                    "SELECT sql FROM sqlite_master WHERE type='table' AND name='profile_revisions'"
                ).fetchone()
                ddl = str(ddl_row["sql"] or "") if ddl_row else ""
                if "check(profile_kind" in ddl.lower():
